      ...
      for (var i = 0; i < res.data.length; i++) {
        ...
        if (item.nickname == userInfo.nickName) {
          ...
          for (var i = 0; i < friends.length; i++) {
            ...
            // 奖杯
            // if (index <= 2) {
            //   var head = writeImage("game/img_di" + (index + 1) + ".png", -180, item_start + item_heigh * (1 - i), 60, 60);
            //   head.anchorY = 0.5;
            //   Laya.stage.addChild(head);
            // }else{
            // }
            var rank = writeText((index + 1).toString(), -180 + item_startX, item_start + item_heigh * (1 - i), 260, 600 + i * 90, 120, 30, 30, "white", "center");
            rank.anchorY = 0.5;
            Laya.stage.addChild(rank);

            // 头像
            var head = writeImage(res.data[index].avatarUrl, -100 + item_startX, item_start + item_heigh * (1 - i), 60, 60);
            head.anchorY = 0.5;
            Laya.stage.addChild(head);
            
            //名字
            var name = writeText(Utils.labelTransform(t_item.nickname, 36, 250), 0 + item_startX, item_start + item_heigh * (1 - i), 260, 600 + i * 90, 120, 30, 30, "white", "center");
            name.anchorY = 0.5;
            Laya.stage.addChild(name);

            var scoreNum = writeText(t_item.KVDataList[0].value, 180 + item_startX, item_start + item_heigh * (1 - i), 410, 600 + i * 90, 120, 30, 30, "#FFFFFF", "left")
