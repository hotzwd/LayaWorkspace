...
function CMenu() {
    ...
    this._init = function() {
        ...
        if (!1 === DISABLE_SOUND_MOBILE || !1 === s_bMobile) f = s_oSpriteLibrary.getSprite("audio_icon"),
    ...
    this._onButPlayRelease = function() {
        this.unload(); ! 1 !== DISABLE_SOUND_MOBILE && !1 !== s_bMobile || createjs.Sound.play("click");
        s_oMain.gotoGame()
    };
    this._init()
}
...
function CInterface() {
    ...
    this._init = function() {
        ...
        h = [];
        v = 800;
        for (q = 0; q < NUM_BULLETS; q++) w = createBitmap(s_oSpriteLibrary.getSprite("bullet")),
        w.x = v,
        w.y = 680,
        s_oStage.addChild(w),
        h[q] = w,
        ...
        if (!1 === DISABLE_SOUND_MOBILE || !1 === s_bMobile) q = s_oSpriteLibrary.getSprite("audio_icon"),
...
function CGame(a) {
    ...
    this.onMouseMove = function(a) { ! 1 !== y && (n = parseInt(a.stageX), r = parseInt(a.stageY))
    };
    ...
    this._initPos = function() {
        for (var a = 51; a < CANVAS_WIDTH;) v.push(new createjs.Point(a, 565)),
        a += CANVAS_WIDTH / 20;
    ...
    this.initDuckObj = function() { ! 1 !== DISABLE_SOUND_MOBILE && !1 !== s_bMobile || createjs.Sound.play("duck_intro");
        if (h === DUCK_ON_SCREEN.length) for (var a = h = 0; a < q.length; a++) q[a].increaseSpeed();
        shuffle(C);
        shuffle(L);
        for (a = 0; a < DUCK_ON_SCREEN[h]; a++) C[a] < v.length / 2 ? q[a].reset(v[C[a]], w[L[a]]) : q[a].reset(v[C[a]], z[L[a]]),
        e++,
        q[a].show();
        k = DUCK_ON_SCREEN[h];
        h++;
